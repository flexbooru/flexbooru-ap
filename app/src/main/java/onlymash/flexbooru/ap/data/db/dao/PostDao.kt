...
interface PostDao {
    ...
    fun getPosts(query: String) : PagingSource<Int, Post>
    ...
    @Query("SELECT MAX(`index`) + 1 FROM `posts` WHERE `query` = :query")
    ...
    @Query("SELECT 1 FROM `posts` WHERE `query` = :query LIMIT 1")
    fun isNotEmpty(query: String): Boolean
}